...
function drawFuzzyGraph(canvasId, type, currentValue) {
    ...
    if (type === 'calories') {
        ...
        datasets = [
            {
                label: 'Low',
                data: xRange.map(x =>  x <= 3 ? 1 : x >= 10 ? 0 : (10 - x) / 7),
                borderColor: '#22c55e',
                backgroundColor: 'rgba(34, 197, 94, 0.3)',
                fill: true,
                tension: 0,
            ...
            },
            {
                label: 'Medium',
                data: xRange.map(x => {
                    if (x <= 3 || x >= 25) return 0;
                    if (x <= 12) return (x - 3) / 9;
                    return (25 - x) / 13;
                }),
                borderColor: '#eab308',
                backgroundColor: 'rgba(234, 179, 8, 0.3)',
            ...
            {
                label: 'High',
                data: xRange.map(x => x >= 25 ? 1 : x <= 15 ? 0 : (x - 15) / 10),
                borderColor: '#ef4444',
                backgroundColor: 'rgba(239, 68, 68, 0.3)',
                fill: true,
                tension: 0,
    ...
    history.forEach((item, index) => {
        ...
        li.innerHTML = `
            ...
            <div class="flex gap-2 text-xs pl-5 overflow-x-auto no-scrollbar">
                ...
                <span class="badge-f px-2 py-1 rounded-lg font-mono font-medium">${item.sugar}g S</span>
            </div>
        `;
